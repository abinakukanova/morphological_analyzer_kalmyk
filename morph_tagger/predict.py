...
def predict_sentence(surface_words, sent, encoder, decoder_lemma, decoder_morph_tags, dataset, device=torch.device("cpu"),
    ...
    # Run encoder
    word_representations, context_aware_representations = encoder(encoded_surfaces.view(1, *encoded_surfaces.size()), sent)
...
def predict(input_file, output_file, dataset_obj_path, encoder_model_path, lemma_decoder_path, morph_decoder_path):
    ...
    with open(output_file, 'w', encoding='UTF-8') as f:
        for sentence, sent in tqdm(data_surface_words):
            conll_sentence = predict_sentence(sentence, sent, encoder, decoder_lemma, decoder_morph_tags,
