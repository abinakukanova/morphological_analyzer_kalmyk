...
def train(language_name, train_data_path, val_data_path, use_min_edit_operation_decoder=True,
    ...
    for epoch in range(num_epochs):
        ...
        for x, y1, y2, y3, y4 in tqdm(train_loader, desc='Training'):
            ...
            y2 = y2.to(device)
            y3 = y3.to(device)
            y4 = ' '.join([item[0] for item in y4])

            # Run encoder
            word_embeddings, context_embeddings = encoder(x, y4)
        ...
        for x, y1, y2, y3, y4 in tqdm(val_loader, desc='Validation'):
            # Skip sentences longer than max_words
            if x.size(1) > max_words:
                continue
